...
In this sense all numbers represented in the machine can be viewed as being modulo 2. We have used this fact to determine our representation of negative numbers. If $x$ is an arbitrary real number, then there is exactly one number $ar{x}$ between 0 and 2 with which it agrees modulo 2, i.e. for each $x$ there is a unique $ar{x}$ such that $0 \le ar{x} < 2$ and $x \equiv ar{x} \pmod 2$. This fact fixes our representation of negative numbers.

---

## Page 15
We agree always to deal with numbers $x$ for which $-1 \le x < 1$. Now the $ar{x}$ associated with $x$ is $x$ if $x \ge 0$; thus, $0 \le ar{x} < 1$ in this case we represent $x by the digitalized form of $ar{x}$. It clearly has $x_0$, its sign digit +, i.e. 0. If $x < 0$, then $ar{x} = x + 2$ and we have $1 \le ar{x} < 2$, i.e. the left-most digit of $ar{x}$ is 1, i.e. -. Thus we always represent a number $x$ by the digitalized form of $ar{x}$ and have the convention that + is 0 and - is 1 with the left-most digit being the sign.
